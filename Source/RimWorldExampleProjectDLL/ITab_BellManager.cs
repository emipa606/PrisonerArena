...
namespace ArenaBell
{
	...
	internal class ITab_BellManager : ITab
	{
		...
		{
			Rect rect = GenUI.ContractedBy(new Rect(0f, 0f, this.size.x, this.size.y), 5f);
			Rect rectTabs = new Rect(rect);
			rectTabs.yMin = rect.yMax;
			rectTabs.height = 550f;
		...
		private void FillTabManager(Rect rect)
		{
			...
			}, label, new Vector2(rect.xMin - 10f, 75f), "Fighter #1", true);
		...
		{
			ITab_BellManager.listingStandard.Begin(rect);
			ITab_BellManager.listingStandard.Gap(50f);
			float offset = GUI.skin.label.CalcSize(new GUIContent("Cool bois")).x / 2f;
			ITab_BellManager.centeredText("Cool bois", new Vector2(rect.xMax / 2f - offset, 165f), 0, 0);
			ITab_BellManager.listingStandard.Gap(50f);
			ITab_BellManager.listingStandard.End();
		}
